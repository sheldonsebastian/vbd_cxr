...
# %% --------------------START HERE
# Resnet18 default parameters + Adam optimizer
import random
...
train_tensorboard_dir = f"{TENSORBOARD_DIR}/2_class_classifier/{fold}/train"
validation_tensorboard_dir = f"{TENSORBOARD_DIR}/2_class_classifier/{fold}/validation"
...
# minority augmentations used for minority class only in train phase
# https://albumentations.ai/docs/api_reference/augmentations/transforms/
minority_transformer = albumentations.Compose([
    # augmentation operations
    albumentations.augmentations.transforms.ColorJitter(brightness=0.5, contrast=0.5,
                                                        saturation=0.5, hue=0.5, always_apply=False,
                                                        p=0.5),
    # horizontal flipping
...
# generic transformer used for majority class in both train and validation
...
train_data_set = VBD_CXR_2_Class_Train(IMAGE_DIR,
                                       MERGED_DIR + "/wbf_merged/k_fold_splits/2_class_classifier"
                                                    "/train_df_5_folds.csv",
                                       majority_transformations=generic_transformer, fold=fold,
                                       minority_class=[1],
                                       minority_transformations=minority_transformer)
