...
print(f"Running for NMS {surname}")

# %% --------------------
# DYNAMIC
train_gt_dataframe = MERGED_DIR + f"/512/nms_merged/90_percent_train/object_detection/90_percent" \
                                  f"/train_df_{surname}.csv"
val_gt_dataframe = MERGED_DIR + f"/512/nms_merged/90_percent_train/object_detection/10_percent" \
                                f"/holdout_df_{surname}.csv"

flag_path = DETECTRON2_DIR + "/faster_rcnn/configurations/v2.yaml"
output_dir = DETECTRON2_DIR + f"/faster_rcnn/output/nms/{surname}"
